		...
		public void CanNotSpecifyReadOnlyProperties()
		{
			var testContent = @"
				...
				namespace TestCase
				{
					public static class Program
					{
						public static PropertyIdentifier<SomethingWithAnId, int> GetIdPropertyUpdater(SomethingWithAnId x)
						{
							return x.GetProperty(_ => _.Id);
						}
					}
			...
			{
				Id = GetPropertyCallAnalyzer.DiagnosticId,
				Message = GetPropertyCallAnalyzer.ReadOnlyPropertyAccessRule.MessageFormat.ToString(),
				Severity = DiagnosticSeverity.Error,
				Locations = new[]
				{
					new DiagnosticResultLocation("Test0.cs", 10, 29)
